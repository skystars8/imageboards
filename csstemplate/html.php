...
function pageStylesheets() {
	...
	// Default stylesheet
	$return .= '<link rel="stylesheet" type="text/css" href="css/' . htmlEscape(TINYIB_DEFAULTSTYLE) . '.css" title="' . htmlEscape($tinyib_stylesheets[TINYIB_DEFAULTSTYLE]) . '" id="mainStylesheet">';
	...
	foreach($tinyib_stylesheets as $filename => $title) {
		...
		$return .= '<link rel="alternate stylesheet" type="text/css" href="css/' . htmlEscape($filename) . '.css" title="' . htmlEscape($title) . '">';
	}
...
function buildPost($post, $res, $compact=false) {
	...
	$nameblock = preg_replace('/\s+[^<]*$/u', '', $post['nameblock']) ?? $post['nameblock'];
...
EOF;
	...
	if (count($tinyib_stylesheets) > 1) {
		...
		foreach($tinyib_stylesheets as $filename => $title) {
			$select_style .= '<option value="' . htmlEscape($filename) . '">'. htmlEscape($title) . '</option>';
		}
...
<img id="captchaimage" src="inc/captcha.php" width="175" height="55" alt="CAPTCHA" onclick="javascript:reloadCAPTCHA()" style="margin-top: 5px;cursor: pointer;"><br><br>';
	...
	$managekey = isset($_GET['manage']) && is_string($_GET['manage']) ? htmlEscape($_GET['manage']) : '';
	return <<<EOF
...
function manageModerationLog($offset) {
	...
	foreach ($logs as $log) {
		...
		$text .= '<tr><td>' . formatDate($log['timestamp']) . '</td><td>' . htmlEscape($u[$log['account']]) . '</td><td>' . $log['message'] . '</td></tr>';
	}
...
function manageReportsPage() {
	...
	foreach ($reports as $report) {
		$post_id = (int)$report['post'];
		if (!isset($report_counts[$post_id])) {
			$report_counts[$post_id] = 0;
			$report_reasons[$post_id] = array();
		}
		$report_counts[$post_id]++;
		if ($report['reason'] != '') {
			$report_reasons[$post_id][] = nl2br(htmlEscape($report['reason']));
		}
	}

	$txt_reported = __('Reported posts');
	$post_html = '';
	foreach ($report_counts as $post_id => $report_count) {
		...
		$post_html .= '<tr><td colspan="2"><small>' . $report_count . ' ' . plural($report_count, __('report'), __('reports')) . '</small></td></tr>';
		if (!empty($report_reasons[$post_id])) {
			$post_html .= '<tr><td colspan="2"><ul><li>' . implode('</li><li>', $report_reasons[$post_id]) . '</li></ul></td></tr>';
		}
...
function manageAccountForm($id = 0) {
	...
	$a['id'] = htmlEscape($a['id']);
	$a['username'] = htmlEscape($a['username']);
...
function manageAccountsTable() {
	...
	if (count($allaccounts) > 0) {
		...
		foreach ($allaccounts as $account) {
			...
			$text .= '<tr><td>' . htmlEscape($account['username']) . '</td><td>';
...
function manageModerateAll($post_ids, $threads, $replies) {
	...
	$post_ids_quoted = htmlEscape(implode(',', $post_ids));
	return <<<EOF
<fieldset>
<legend>$txt_moderate</legend>
<table border="0" cellspacing="0" cellpadding="0" width="100%">
...
function manageEditKeyword($id) {
	...
	if ($id > 0) {
		...
		$v_text = htmlEscape($keyword['text']);
...
function manageKeywordsTable() {
	...
	if (count($keywords) > 0) {
		...
		foreach ($keywords as $keyword) {
			...
			$text .= '<tr><td>' . htmlEscape($keyword['text']) . '</td><td>' . $action . '</td><td><a href="?manage&keywords=' . $keyword['id'] . '">' . __('Edit') . '</a> <a href="?manage&keywords&deletekeyword=' . $keyword['id'] . '">' . __('Delete') . '</a></td></tr>';
		}
		$text .= '</table>';
	}
	return $text;
