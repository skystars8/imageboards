...
-- migrate:split
CREATE INDEX posts_ip_hash ON posts(ip_hash);
-- migrate:split
...
CREATE TABLE citations (
    ...
    PRIMARY KEY (post_id, target_post_id),
    FOREIGN KEY (post_id) REFERENCES posts(id) ON DELETE CASCADE,
    FOREIGN KEY (target_post_id) REFERENCES posts(id) ON DELETE CASCADE
);
-- migrate:split
CREATE INDEX citations_target ON citations(target_post_id, post_id);
-- migrate:split
...
CREATE TABLE bans (
    ...
    FOREIGN KEY (created_by) REFERENCES moderators(id) ON DELETE RESTRICT
);
-- migrate:split
CREATE INDEX bans_ip_active ON bans(ip_hash, expires_at);
-- migrate:split
