...
try {
    ...
    check($applied === ['001_initial'], 'initial migration is applied');
    ...
    $board = $boards->create('chess', 'Chess', 'Test board');
    ...
    $moderatorId = $moderation->createModerator(
        'admin',
        password_hash('a-long-test-password', PASSWORD_DEFAULT),
    );
    check($moderatorId > 0, 'a moderator can be created');
