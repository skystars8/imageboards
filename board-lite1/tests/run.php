...
try {
    ...
    check(
        str_contains($response->body(), '<details class="composer composer--thread">') &&
        !str_contains($response->body(), '<details class="composer composer--thread" open'),
        'the new-thread form stays compact and collapsed by default',
    );
    ...
    check(
        ...
        str_contains($threadResponse->body(), '<details id="reply" class="composer composer--reply">') &&
        str_contains($threadResponse->body(), 'rows="4"'),
        'the thread reply form uses the collapsed compact composer',
    );
