...
final readonly class View
{
    ...
    public function __construct(
        ...
        $themeGroups = require $this->config->rootPath() . '/config/themes.php';
        if (!is_array($themeGroups)) {
            throw new RuntimeException('config/themes.php must return an array.');
        }
    ...
    public function captchaChallenge(string $purpose): array
    {
        return $this->captcha->challenge($purpose);
    }

    public function time(int|string $timestamp): string
    {
        return date('m/d/y (D) H:i:s', (int) $timestamp);
    }
