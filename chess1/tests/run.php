...
try {
    ...
    check($installation['migrations'] === ['001_initial', '002_remove_client_tracking', '003_remove_quote_references'], 'browser setup applies all migrations');
    check(
    ...
    $legacyTables = $database->pdo()->query(
        "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('bans', 'rate_limits', 'citations')"
    )->fetchAll(PDO::FETCH_COLUMN);
    check($legacyTables === [], 'ban, IP-backed rate-limit, and quote-reference tables are absent');
    ...
    $replyBody = 'I prefer 2...d6. <script>alert(1)</script>';
    $reply = $posts->create(
        $board,
        1,
        '',
    ...
    check(!str_contains($rendered, 'post-reference'), 'posts do not create quote-reference links');
...
[pgn]1. e4 c5 2. Nf3[/pgn]",
    ...
    $captcha = new Captcha();
    $challenge = $captcha->challenge('report:chess:1');
    preg_match('/(\d+) \+ (\d+)/', $challenge['question'], $captchaNumbers);
    $captchaAnswer = (string) ((int) ($captchaNumbers[1] ?? 0) + (int) ($captchaNumbers[2] ?? 0));
    $captcha->validate($challenge['token'], $captchaAnswer, 'report:chess:1');
    $reusedCaptchaRejected = false;
    try {
        $captcha->validate($challenge['token'], $captchaAnswer, 'report:chess:1');
    } catch (HttpException $error) {
        $reusedCaptchaRejected = $error->status === 422;
    }
    check($reusedCaptchaRejected, 'report CAPTCHA challenges are correct, short-lived, and one-time');
    ...
    check(
        ...
        str_contains($response->body(), 'id="theme-stylesheet"') &&
        str_contains($response->body(), '/assets/js/app.js') &&
        str_contains($response->body(), '<option value="checkmate">Checkmate</option>') &&
        str_contains($response->body(), '<option value="midnight-mate">Midnight Mate</option>') &&
        str_contains($response->body(), '<optgroup label="Traditional themes">') &&
        str_contains($response->body(), '<option value="yotsuba">Yotsuba</option>') &&
        str_contains($response->body(), '<optgroup label="Experimental themes">') &&
        substr_count($response->body(), '<option value=') === 36 &&
        str_contains($response->body(), 'id="text-smaller"') &&
        str_contains($response->body(), 'id="text-larger"') &&
        str_contains($response->body(), 'id="text-weight"'),
        'the board includes 36 modular themes and persistent reading controls',
    );
    check(
        !str_contains($response->body(), '>Permalink</a>') &&
        !str_contains($response->body(), '>Quote</a>') &&
        str_contains($response->body(), '>Report</summary>') &&
        str_contains($response->body(), 'name="captcha_token"') &&
        str_contains($response->body(), 'name="captcha_answer"'),
        'the public post menu contains reporting with a built-in CAPTCHA and no permalink or quote actions',
    );
    check(
