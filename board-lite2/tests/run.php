...
try {
    ...
    check(
        str_contains($response->body(), '<section class="post-area"') &&
        str_contains($response->body(), '<table class="post-form__table">') &&
        str_contains($response->body(), '>New Thread</button>'),
        'the board renders the traditional compact posting table',
    );
    check(
    ...
    check(
        ...
        str_contains($threadResponse->body(), '<section id="reply" class="post-area"') &&
        str_contains($threadResponse->body(), 'Posting mode: Reply') &&
        str_contains($threadResponse->body(), 'rows="4"'),
        'the thread uses the traditional reply-mode form',
    );
