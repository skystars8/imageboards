...
final readonly class ModeratorController
{
    ...
    public function login(Request $request, array $parameters): Response
    {
        ...
        $this->rateLimiter->assertAllowed(
            $identityHash,
            'login',
            $this->config->requireInt('login_rate_window'),
            $this->config->requireInt('login_rate_limit'),
        );
    ...
    public function banPost(Request $request, array $parameters): Response
    {
        $moderator = $this->requireWrite($request);
        $board = $this->requireBoard($parameters['board']);
        $post = $this->requirePost($board, (int) $parameters['post']);
        $reason = $this->banReason($request);
        $duration = $this->banDuration($request);
        $banId = $this->moderation->createBan(
            $post['ip_hash'],
            $reason,
            $duration === 0 ? null : time() + $duration,
            (int) $moderator['id'],
        );
        $this->moderation->log(
            (int) $moderator['id'],
            ...
            (int) $board['id'],
            (int) $post['id'],
            sprintf('Ban #%d: %s', $banId, $reason),
        );
        Session::flash('success', 'Post author banned.');
    ...
    public function bans(Request $request, array $parameters): Response
    {
        $this->auth->requireUser();

        return Response::html($this->view->render('mod/bans', [
            'title' => 'Bans',
            'bans' => $this->moderation->bans(),
        ]));
    }

    public function createBan(Request $request, array $parameters): Response
    {
        $moderator = $this->requireWrite($request);
        $ip = $request->input('ip');
        if (filter_var($ip, FILTER_VALIDATE_IP) === false) {
            throw new HttpException(422, 'Enter a valid IPv4 or IPv6 address.');
        }

        $reason = $this->banReason($request);
        $duration = $this->banDuration($request);
        $banId = $this->moderation->createBan(
            $this->identity->hashIp($ip),
            $reason,
            $duration === 0 ? null : time() + $duration,
            (int) $moderator['id'],
        );
        $this->moderation->log(
            (int) $moderator['id'],
            'create-ban',
            details: sprintf('Ban #%d: %s', $banId, $reason),
        );
        Session::flash('success', 'Ban created.');

        return Response::redirect($this->view->url('/mod/bans'));
    }

    public function deleteBan(Request $request, array $parameters): Response
    {
        $moderator = $this->requireWrite($request);
        $banId = (int) $parameters['ban'];
        $this->moderation->deleteBan($banId);
        $this->moderation->log((int) $moderator['id'], 'delete-ban', details: 'Ban #' . $banId);
        Session::flash('success', 'Ban removed.');
    ...
    private function banReason(Request $request): string
    {
        $reason = $request->input('reason');
        if ($reason === '' || mb_strlen($reason) > 500) {
            throw new HttpException(422, 'Give a ban reason of 500 characters or fewer.');
        }

        return $reason;
    }

    private function banDuration(Request $request): int
    {
        $duration = (int) $request->input('duration', '86400');
        if (!in_array($duration, self::BAN_DURATIONS, true)) {
            throw new HttpException(422, 'Choose a valid ban duration.');
        }
