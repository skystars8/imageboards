...
/**
 * Display a fatal error page and exit.
 *
 * @param string $message Error text (escaped for HTML unless $allow_html is true)
 * @param bool|int $priority Syslog priority, or false to skip syslog
 * @param array $debug_stuff Extra debug info when config.debug is on
 * @param bool $allow_html When true, $message may contain trusted HTML (e.g. captcha reload script)
 */
function error($message, $priority = true, $debug_stuff = [], $allow_html = false) {
	global $board, $mod, $config, $db_error;

	if (!empty($config['syslog']) && $priority !== false) {
		// Use LOG_NOTICE instead of LOG_ERR or LOG_WARNING because most error message are not significant.
		_syslog($priority !== true ? $priority : LOG_NOTICE, is_string($message) ? strip_tags($message) : $message);
	}
	...
	// JSON always gets a plain string (no HTML)
	$json_message = is_string($message) ? trim(html_entity_decode(strip_tags($message), ENT_QUOTES, 'UTF-8')) : $message;
	...
	// Escape by default; trusted HTML only when explicitly requested
	$safe_message = $allow_html ? $message : utf8tohtml((string)$message);
