...
final readonly class ModeratorController
{
    ...
    public function editPost(Request $request, array $parameters): Response
    {
        $moderator = $this->requireWrite($request);
        $board = $this->requireBoard($parameters['board']);
        $post = $this->requirePost($board, (int) $parameters['post']);
        if ((int) $post['is_deleted'] === 1) {
            throw new HttpException(410, 'Deleted posts cannot be edited.');
        }
        ...
        $replacement = $this->uploads->process($request->file('image'));
        $removeImage = $request->input('remove_image') === '1';
        $keepsExistingImage = ($post['stored_name'] ?? null) !== null && !$removeImage && $replacement === null;
        if ($body === '' && !$keepsExistingImage && $replacement === null) {
            $this->uploads->remove($replacement);
            throw new HttpException(422, 'A post needs text or an image.');
        }

        try {
            $oldAttachment = $this->posts->update(
                (int) $post['id'],
                $subject,
                $name,
                $body,
                $replacement,
                $removeImage || $replacement !== null,
                $this->markup->references($body, $board['slug']),
            );
        ...
        $this->uploads->remove($oldAttachment);
        $this->moderation->log(
            (int) $moderator['id'],
            'edit-post',
            (int) $board['id'],
            (int) $post['id'],
            sprintf('/%s/ No.%d%s', $board['slug'], $post['post_no'], $replacement !== null ? ' (image replaced)' : ($removeImage ? ' (image removed)' : '')),
        );
