...
final readonly class PostRepository
{
    ...
    public function update(
        ...
        return $this->database->immediate(function (PDO $pdo) use (
            ...
            $postQuery = $pdo->prepare('SELECT id, is_deleted FROM posts WHERE id = :id LIMIT 1');
            ...
            if ($replaceAttachment) {
                ...
                $existing = $attachmentQuery->fetch();
                $oldAttachment = $existing === false ? null : $existing;
            ...
            $update = $pdo->prepare(
                'UPDATE posts
                 SET subject = :subject, name = :name, body = :body
                 WHERE id = :id'
            );
            $update->bindValue(':subject', $subject === '' ? null : $subject, $subject === '' ? PDO::PARAM_NULL : PDO::PARAM_STR);
            $update->bindValue(':name', $name);
            $update->bindValue(':body', $body);
            $update->bindValue(':id', $postId, PDO::PARAM_INT);
            $update->execute();

            $deleteCitations = $pdo->prepare('DELETE FROM citations WHERE post_id = :post_id');
            $deleteCitations->execute(['post_id' => $postId]);
            $this->insertCitations($pdo, $postId, $references);
