...
function mkhash(string $username, $password = null, $salt = false) {
	...
	$ip = (!empty($config['mod']['lock_ip']) && !empty($_SERVER['REMOTE_ADDR']))
		? $_SERVER['REMOTE_ADDR']
		: '';

	// HMAC-based session token (replaces md5/sha1 nesting + rand())
	$hash = substr(base64_encode(hash_hmac(
		'sha256',
		$username . '|' . $password . '|' . $salt . '|' . $ip . '|' . $config['password_crypt_version'],
		$config['cookies']['salt'],
		true
	)), 0, 20);
...
function modLog(string $action, ?string $_board = null): void {
	...
	$query->bindValue(':ip', get_ip_for_storage());
