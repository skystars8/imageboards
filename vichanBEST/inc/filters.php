				...
				foreach ($this->flood_check as $flood_post) {
					foreach ($match as $flood_match_arg) {
						switch ($flood_match_arg) {
							case 'ip':
								// Match flood key (real IP or privacy HMAC), never log raw address here
								if ($flood_post['ip'] != get_flood_key()) {
									continue 3;
								}
								break;
							case 'body':
	...
	public function action() {
		...
		if (isset($this->action)) {
			switch($this->action) {
				...
				case 'ban':
					if (!privacy_store_ip()) {
						// IP bans unavailable — reject post only
					...
					Bans::new_ban(get_ip_for_storage(), $this->reason, $this->expires, $this->all_boards ? false : $board['uri'], -1);
