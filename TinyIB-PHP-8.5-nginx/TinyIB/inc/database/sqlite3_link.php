...
try {
	...
	$db->exec("UPDATE $keywords_table SET action = 'delete' WHERE action NOT IN ('report', 'hide', 'delete')");
	...
	if (sqliteColumnExists($db, TINYIB_DBREPORTS, 'ip')) {
		$migrated_reports_name = TINYIB_DBREPORTS . '_privacy_migration';
		$migrated_reports_table = sqliteIdentifier($migrated_reports_name);
		$reason_expression = sqliteColumnExists($db, TINYIB_DBREPORTS, 'reason') ? 'reason' : "''";

		$db->exec('BEGIN IMMEDIATE');
		try {
			$db->exec("DROP TABLE IF EXISTS $migrated_reports_table");
			$db->exec("CREATE TABLE $migrated_reports_table (
				id INTEGER PRIMARY KEY,
				post INTEGER NOT NULL,
				reason TEXT NOT NULL
			)");
			$db->exec("INSERT INTO $migrated_reports_table (id, post, reason)
				SELECT id, post, $reason_expression FROM $reports_table");
			$db->exec("DROP TABLE $reports_table");
			$db->exec("ALTER TABLE $migrated_reports_table RENAME TO $reports_table");
			$db->exec('COMMIT');
		} catch (Throwable $error) {
			$db->exec('ROLLBACK');
			throw $error;
		}
	} elseif (!sqliteColumnExists($db, TINYIB_DBREPORTS, 'reason')) {
		$db->exec("ALTER TABLE $reports_table ADD COLUMN reason TEXT NOT NULL DEFAULT ''");
	}
