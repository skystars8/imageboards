...
function _strlen(string $string): int {
	return mb_strlen($string);
}

function _strpos(string $haystack, string $needle, int $offset = 0): int|false {
	return mb_strpos($haystack, $needle, $offset);
}

function _substr(string $string, int $start, ?int $length = null): string {
	return mb_substr($string, $start, $length);
}

function _substr_count(string $haystack, string $needle): int {
...
/**
 * Validate the simple image CAPTCHA when enabled.
 * $mode is truthy (typically 'simple') to require CAPTCHA, or empty to skip.
 */
function checkCAPTCHA($mode): void {
	if (!$mode) {
		return;
	}

	$captcha = isset($_POST['captcha']) ? strtolower(trim((string)$_POST['captcha'])) : '';
	$captcha_solution = isset($_SESSION['tinyibcaptcha']) ? strtolower(trim((string)$_SESSION['tinyibcaptcha'])) : '';
...
function checkMessageSize() {
	...
	if (TINYIB_MAXMESSAGE > 0 && _strlen($message) > TINYIB_MAXMESSAGE) {
		fancyDie(sprintf(__('Please shorten your message, or post it in multiple parts. Your message is %1$d characters long, and the maximum allowed is %2$d.'), _strlen($message), TINYIB_MAXMESSAGE));
	}
}
...
function createThumbnail($file_location, $thumb_location, $new_w, $new_h, $spoiler) {
	...
	if (TINYIB_THUMBNAIL == 'gd' || (TINYIB_THUMBNAIL == 'ffmpeg' && preg_match("/jpg|jpeg/", $system[0]))) {
		...
		// GdImage resources are freed automatically (imagedestroy is deprecated in PHP 8.5)
...
/**
 * Format a Unix timestamp using TINYIB_DATEFMT (DateTime format string).
 * @see https://www.php.net/manual/en/datetime.format.php
 */
function formatDate($timestamp): string {
	$dt = (new DateTimeImmutable('@' . (int)$timestamp))->setTimezone(new DateTimeZone(date_default_timezone_get()));
	return $dt->format(TINYIB_DATEFMT);
}
