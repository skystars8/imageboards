...
function manageLogInForm() {
	...
	$captcha_inner_html = '';
	if (TINYIB_MANAGECAPTCHA) { // Simple CAPTCHA
		$captcha_inner_html = '
<br>
<input type="text" name="captcha" id="captcha" size="6" accesskey="c" autocomplete="off">&nbsp;&nbsp;' . __('(enter the text below)') . '<br>
...
function manageReportsPage() {
	...
	foreach ($posts as $post) {
		...
		$report_info = $post['report_count'] . ' ' . plural($post['report_count'], __('report'), __('reports'));
		$post_html .= '<tr><td colspan="2"><small>' . $report_info . '</small></td></tr>';
