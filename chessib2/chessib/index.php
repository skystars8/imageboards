    ...
    <form action="post.php" method="post" enctype="multipart/form-data">
      ...
      <div class="form-grid">
        <div class="form-row">
          ...
          <input type="text" id="name" name="name" maxlength="<?= MAX_NAME_LENGTH ?>" placeholder="<?= e(DEFAULT_NAME) ?> (#trip)">
        </div>
        <div class="form-row">
          <label for="subject">Subject</label>
          <input type="text" id="subject" name="subject" maxlength="<?= MAX_SUBJECT_LENGTH ?>" placeholder="Optional">
        </div>
        <div class="form-row full">
          <label for="comment">Comment <span style="color:var(--danger)">*</span></label>
          <textarea id="comment" name="comment" required maxlength="<?= MAX_COMMENT_LENGTH ?>" placeholder="Openings, tactics, positions, puzzles..."></textarea>
        </div>
        <?php if (ALLOW_IMAGES): ?>
        <div class="form-row">
      ...
      <div class="form-actions">
        ...
        <span class="hint">**bold** *italic* ||spoiler|| &gt;greentext &gt;&gt;123 · max <?= human_size(MAX_FILE_SIZE) ?></span>
      </div>
    </form>
  </section>
