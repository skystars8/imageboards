...
rem Prefer the folder containing this batch file.
if exist "%APP_DIR%public\router.php" goto app_found

rem Fall back to the folder used during the original setup.
set "APP_DIR=C:\tinyib\ib1\"
if exist "%APP_DIR%public\router.php" goto app_found

echo.
echo  ERROR: Chessboard Lite was not found.
echo.
echo  Put this batch file inside the Chessboard Lite folder,
echo  alongside README.md and the public folder, then run it again.
echo.
pause
exit /b 1
...
:php_found
"%PHP_EXE%" -r "exit(PHP_VERSION_ID >= 80500 && extension_loaded('pdo_sqlite') && extension_loaded('mbstring') && extension_loaded('fileinfo') && extension_loaded('gd') && extension_loaded('session') && extension_loaded('json') ? 0 : 1);" >nul 2>&1
if errorlevel 1 (
    echo.
    echo  ERROR: Chessboard Lite needs PHP 8.5 or newer with:
