...
final readonly class Installer
{
    ...
    public function install(string $username, string $password): array
    {
        ...
        try {
            ...
            $keyCreated = $this->ensureApplicationKey();
    ...
    {
        $path = $this->config->requireString('key_path');
        if (!is_file($path)) {
            return false;
        }
        ...
        return is_string($contents) && strlen(trim($contents)) >= 32;
    }
    ...
    private function ensureApplicationKey(): bool
    {
        ...
        if (is_file($path)) {
            if (!$this->hasValidKey()) {
                throw new RuntimeException('The existing application key is invalid.');
            }
        ...
        if ($stream === false) {
            ...
            throw new RuntimeException('Unable to create the application key.');
        }

        $key = bin2hex(random_bytes(32)) . "\n";
        try {
            if (fwrite($stream, $key) !== strlen($key) || !fflush($stream)) {
                throw new RuntimeException('Unable to write the application key.');
            }
        } finally {
            fclose($stream);
        }
