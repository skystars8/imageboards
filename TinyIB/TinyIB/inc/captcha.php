...
if (!extension_loaded('gd')) {
	...
	die('The GD extension is required for the simple CAPTCHA.');
}
...
class SimpleCaptcha {
	...
	/** Allocated GD colors and the final text position */
	public $GdBgColor;
	public $GdFgColor;
	public $GdShadowColor;
	public $textFinalX = 0;
	public $freeTypeAvailable = null;
	...
	protected function ImageAllocate() {
		...
		// Foreground color
		$color = $this->colors[random_int(0, sizeof($this->colors) - 1)];
	...
	protected function GetRandomCaptchaText($length = null) {
		...
		$text = "";
		$vocal = random_int(0, 1);
		for ($i = 0; $i < $length; $i++) {
			if ($vocal) {
				$text .= substr($vocals, random_int(0, 4), 1);
			} else {
				$text .= substr($words, random_int(0, 22), 1);
			}
			$vocal = !$vocal;
		}
		return $text;
	...
	function GetDictionaryCaptchaText($extended = false) {
		...
		$file_size = filesize($wordsfile);
		$max_line = is_int($file_size) ? (int)floor($file_size / $length) - 2 : 0;
		if ($max_line < 1) {
			fclose($fp);
			return false;
		}
		$line = random_int(1, $max_line);
		...
		if ($extended) {
			...
			foreach ($text as $i => $char) {
				if (random_int(0, 1) && in_array($char, $vocals)) {
					$text[$i] = $vocals[random_int(0, 4)];
				}
			}
			$text = implode('', $text);
		}
	...
	protected function WriteLine() {
		$x1 = (int)round($this->width * $this->scale * .15);
		$x2 = (int)round($this->textFinalX);
		$min_y = (int)round($this->height * $this->scale * .40);
		$max_y = (int)round($this->height * $this->scale * .65);
		$y1 = random_int($min_y, $max_y);
		$y2 = random_int($min_y, $max_y);
		$width = max(1, (int)round($this->lineWidth / 2 * $this->scale));
	...
	protected function WriteText($text, $fontcfg = array()) {
		if (!$this->HasFreeType()) {
			...
			$x = max(5, (int)round(($this->width - $text_width) / 2));
			$y = max(5, (int)round(($this->height - $text_height) / 2));
			imagestring($this->im, $font, $x, $y, $text, $this->GdFgColor);
			$this->textFinalX = $x + $text_width;
			return;
		}
		...
		$length = strlen($text);
		for ($i = 0; $i < $length; $i++) {
			$degree = random_int($this->maxRotation * -1, $this->maxRotation);
	...
	protected function HasFreeType() {
		if ($this->freeTypeAvailable !== null) {
			return $this->freeTypeAvailable;
		}

		$test_font = __DIR__ . '/fonts/roboto_regular.ttf';
		$this->freeTypeAvailable = function_exists('imagettfbbox')
			&& @imagettfbbox(10, 0, $test_font, 'A') !== false;
		return $this->freeTypeAvailable;
	}
	...
	protected function WaveImage() {
		...
		for ($i = 0; $i < ($this->width * $this->scale); $i++) {
			imagecopy($this->im, $this->im,
				$i - 1, (int)round(sin($k + $i / $xp) * ($this->scale * $this->Xamplitude)),
		...
		for ($i = 0; $i < ($this->height * $this->scale); $i++) {
			imagecopy($this->im, $this->im,
				(int)round(sin($k + $i / $yp) * ($this->scale * $this->Yamplitude)), $i - 1,
