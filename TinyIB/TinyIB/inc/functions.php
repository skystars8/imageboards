...
function verifyRemoteCaptcha($url, $secret, $response) {
	$verify = curl_init($url);
	if ($verify === false) {
		fancyDie(__('Unable to initialize CAPTCHA verification.'));
	}
	...
	$response_code = curl_getinfo($verify, CURLINFO_HTTP_CODE);
	unset($verify);

	if (!is_string($verify_response) || $response_code !== 200) {
		return false;
	}

	$response_data = json_decode($verify_response, true);
	return is_array($response_data) && ($response_data['success'] ?? false) === true;
}

function checkCAPTCHA($mode) {
	if ($mode === 'hcaptcha') {
		$captcha = isset($_POST['h-captcha-response']) && is_string($_POST['h-captcha-response']) ? $_POST['h-captcha-response'] : '';
		if ($captcha == '') {
			fancyDie('Failed CAPTCHA. Reason:<br>Please click the checkbox labeled "I am human".');
		}

		if (!verifyRemoteCaptcha('https://api.hcaptcha.com/siteverify', TINYIB_HCAPTCHA_SECRET, $captcha)) {
			fancyDie('Failed CAPTCHA.');
		}
	} else if ($mode === 'recaptcha') {
		$captcha = isset($_POST['g-recaptcha-response']) && is_string($_POST['g-recaptcha-response']) ? $_POST['g-recaptcha-response'] : '';
		if ($captcha == '') {
			fancyDie('Failed CAPTCHA. Please click the checkbox labeled "I\'m not a robot".');
		}

		if (!verifyRemoteCaptcha('https://www.google.com/recaptcha/api/siteverify', TINYIB_RECAPTCHA_SECRET, $captcha)) {
			fancyDie('Failed CAPTCHA.');
		}
...
function formatDate($timestamp) {
	return date(TINYIB_DATEFMT, (int)$timestamp);
}
