...
function mod_log(Context $ctx, $page_no = 1) {
	...
	$query = prepare("SELECT `username`, `mod`, `board`, `time`, `text` FROM ``modlogs`` LEFT JOIN ``mods`` ON `mod` = ``mods``.`id` ORDER BY `time` DESC LIMIT :offset, :limit");
...
function mod_user_log(Context $ctx, $username, $page_no = 1) {
	...
	$query = prepare("SELECT `username`, `mod`, `board`, `time`, `text` FROM ``modlogs`` LEFT JOIN ``mods`` ON `mod` = ``mods``.`id` WHERE `username` = :username ORDER BY `time` DESC LIMIT :offset, :limit");
...
function mod_board_log(Context $ctx, $board, $page_no = 1, $hide_names = false, $public = false) {
	...
	$query = prepare("SELECT `username`, `mod`, `board`, `time`, `text` FROM ``modlogs`` LEFT JOIN ``mods`` ON `mod` = ``mods``.`id` WHERE `board` = :board ORDER BY `time` DESC LIMIT :offset, :limit");
...
function mod_user_promote(Context $ctx, $uid, $action) {
	...
	// Do not clobber global $mod (logged-in moderator) — use a local target row.
	if (!$target = $query->fetch(PDO::FETCH_ASSOC))
		error($config['error']['404']);
	...
	foreach ($groups as $group_value => $group_name) {
		if ($action == 'promote' && $group_value > $target['type']) {
			$new_group = $group_value;
			break;
		} elseif ($action == 'demote' && $group_value < $target['type']) {
			$new_group = $group_value;
			break;
		}
	}
	...
	modLog(($action == 'promote' ? 'Promoted' : 'Demoted') . ' user "' .
		utf8tohtml($target['username']) . '" to ' . $config['mod']['groups'][$new_group]);
...
function mod_report_dismiss(Context $ctx, $id, $action) {
	...
	switch ($action) {
		case '&post':
		case '&all': // alias: dismiss all reports for this post
			if (!hasPermission($config['mod']['report_dismiss_post'], $board)
				&& !hasPermission($config['mod']['report_dismiss'], $board)) {
				error($config['error']['noaccess']);
			}
			$query = prepare('DELETE FROM ``reports`` WHERE `post` = :post AND `board` = :board');
			$query->bindValue(':post', $post);
			$query->bindValue(':board', $board);
			modLog("Dismissed all reports for post #{$post}", $board);
			break;
		case '':
		default:
